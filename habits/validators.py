...
class HabitValidator:
    ...
    def validate_time_success(self, attrs):
        ...
        if time_success is None:
            return
        if time_success > 120:
            raise serializers.ValidationError({"time_success": "Время выполнения не должно превышать 120 секунд."})
    ...
    def validate_reward_and_related(self, attrs):
        ...
        if attrs.get("related_habit") and attrs.get("reward"):
            raise serializers.ValidationError("Нельзя одновременно выбирать и связанную привычку и вознаграждение.")
